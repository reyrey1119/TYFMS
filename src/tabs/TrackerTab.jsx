...
// Threading: milestones already covered by other tools get checked off automatically
// once the shared profile shows the work is done.
useEffect(() => {
if (autoApplied || !profile) return
const auto = []
if (profile.mos && !checked.includes('m7')) auto.push('m7')
if (profile.target_industries && profile.target_industries.length >= 2 && !checked.includes('m9')) auto.push('m9')
if (auto.length > 0) saveChecked([...checked, ...auto])
setAutoApplied(true)
}, [profile])
...
{profile && (profile.mos || (profile.target_industries && profile.target_industries.length > 0)) && (
<div style={{
...
<strong>Connected to your profile.</strong> Milestones you've already completed in the Skills Translator are checked off automatically.
</p>
</div>
)}
