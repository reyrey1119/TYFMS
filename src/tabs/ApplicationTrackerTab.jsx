...
'No Response': { bg: '#EFE7CD', color: '#5C5646' },
}
...
const fieldStyle = {
width: '100%', fontSize: 12, borderRadius: 6, border: '1px solid #C2B588',
...
<strong>Your target industries:</strong> {profile.target_industries.join(', ')} — from your Skills Translator results. Keep your search focused here.
</p>
</div>
)}
...
<div key={s.label} style={{ background: '#fff', border: '1px solid #D6CBA3', borderRadius: 12, padding: '14px 16px', textAlign: 'center' }}>
...
<button onClick={() => setAddingNew(false)} style={{ padding: '7px 14px', background: '#fff', border: '1px solid #C2B588', borderRadius: 8, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', color: '#5C5646' }}>
Cancel
</button>
</div>
</div>
...
{loading ? (
<div style={{ textAlign: 'center', padding: '32px 16px', color: '#8F8768' }}>
<p style={{ fontSize: 13 }}>Loading applications…</p>
</div>
) : apps.length === 0 ? (
<div style={{ textAlign: 'center', padding: '40px 16px', background: '#fff', border: '1px solid #D6CBA3', borderRadius: 16 }}>
<p style={{ fontSize: 28, marginBottom: 10 }}>📋</p>
<p style={{ fontSize: 14, fontWeight: 600, color: '#211F19', marginBottom: 6 }}>No applications logged yet</p>
<p style={{ fontSize: 13, color: '#5C5646', lineHeight: 1.6 }}>Click "Log application" to start tracking your job search.</p>
</div>
) : (
<div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
{apps.map(app => (
<div key={app.id} style={{ background: '#fff', border: '1px solid #D6CBA3', borderRadius: 12, padding: '14px 16px' }}>
{editingId === app.id ? (
<div>
...
<button onClick={() => setEditingId(null)} style={{ padding: '5px 12px', background: '#fff', border: '1px solid #C2B588', borderRadius: 7, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', color: '#5C5646' }}>
Cancel
</button>
</div>
</div>
...
<p style={{ fontSize: 14, fontWeight: 700, color: '#211F19' }}>{app.job_title || '—'}</p>
<span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: STATUS_COLORS[app.status]?.bg || '#EFE7CD', color: STATUS_COLORS[app.status]?.color || '#5C5646', fontWeight: 600 }}>{app.status}</span>
</div>
<p style={{ fontSize: 12, color: '#5C5646', marginBottom: 2 }}>{app.company || '—'}</p>
<div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
{app.date_applied && <p style={{ fontSize: 11, color: '#8F8768' }}>Applied: {app.date_applied}</p>}
{app.follow_up_date && <p style={{ fontSize: 11, color: '#C07A28' }}>Follow-up: {app.follow_up_date}</p>}
{app.notes && <p style={{ fontSize: 11, color: '#5C5646', fontStyle: 'italic' }}>{app.notes.slice(0, 80)}{app.notes.length > 80 ? '…' : ''}</p>}
</div>
</div>
<div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
<button onClick={() => startEdit(app)} style={{ padding: '4px 10px', background: '#fff', border: '1px solid #C2B588', borderRadius: 7, fontSize: 11, cursor: 'pointer', fontFamily: 'inherit', color: '#5C5646' }}>Edit</button>
...
<button onClick={() => setDeleteConfirm(null)} style={{ padding: '4px 8px', background: '#fff', border: '1px solid #C2B588', borderRadius: 7, fontSize: 11, cursor: 'pointer', fontFamily: 'inherit', color: '#5C5646' }}>Cancel</button>
</>
) : (
<button onClick={() => setDeleteConfirm(app.id)} style={{ padding: '4px 8px', background: '#fff', border: '1px solid #C2B588', borderRadius: 7, fontSize: 11, cursor: 'pointer', fontFamily: 'inherit', color: '#a32d2d' }}>×</button>
)}
</div>
</div>
)}
...
{!useDb && (
<p style={{ fontSize: 11, color: '#8F8768', marginTop: 16, lineHeight: 1.6, textAlign: 'center' }}>
